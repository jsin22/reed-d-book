...
Skipped unless REEDD_INTEGRATION=1, because it needs torch/pocket_tts/spacy/
ffmpeg and takes a minute or two even on the short sample. Everything else in
this suite runs against a fake. Still no broker: the task is applied in-process.
...
class RealConversionTest(TempDataDirTestCase):
    def test_produces_a_playable_audiobook_and_a_usable_sync_file(self):
        ...
        manifest = store.create(SAMPLE.name, 'alba', 1.0, 'pocket_tts')
